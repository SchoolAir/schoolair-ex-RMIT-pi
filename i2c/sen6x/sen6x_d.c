...
// Python verification routine sends SIGUSR1 to request an immediate sample.
// The flag is checked every 100 ms during the normal 60 s sleep so the daemon
// responds within ~100 ms without needing interruptible sleep calls.
static volatile sig_atomic_t g_force_sample = 0;
...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        // Sleep in 100 ms steps so SIGUSR1 can break it within ~100 ms.
        for (int i = 0; i < (int)(sleeptime * 10) && !g_force_sample; i++)
            sensirion_hal_sleep_us(100000UL);
        g_force_sample = 0;
